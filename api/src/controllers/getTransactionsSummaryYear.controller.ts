...
import type { GetTransactionsSummaryQueryYear } from "../schemas/transaction.schema";
...
export const getTransactionsSummaryYear = async (
  request: FastifyRequest<{ Querystring: GetTransactionsSummaryQueryYear }>,
  reply: FastifyReply
): Promise<void> => {
  ...
  try {
    ...
    for (const transaction of transactions) {
      if (transaction.type === TransactionType.expense) {
        const existingExpenses = groupedExpenses.get(
