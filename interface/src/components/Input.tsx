...
const Input = ({icon, fullWidth, error, label, id, className, ...rest}:InputProps) => {
    ...
    return(
        <div className={`${fullWidth ? "w-full": ""} mb-4`}>
            ...
            <input id={inputId} className={`block w-full rounded-xl border ${error ? "border-red-500" : "border-gray-700"} bg-gray-800 px-4 py-3 text-sm text-gray-50 transition-all focus:outline-none focus:ring-2 ${error ? "focus:border-red-500 focus:ring-red-500/2" : "focus:border-primary-500 focus:ring-primary-500/2"} ${icon ? "pl-10" : ""} ${className}`}{...rest} />
