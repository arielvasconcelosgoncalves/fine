...
const Transactions = () => {
  ...
  const [transactionsYear, setTransactionsYear] = useState<Transaction[]>([]);
  const [filteredTransactions, setFilteredTransactions] = useState<
    Transaction[]
  >([]);
  const [filteredTransactionsYear, setFilteredTransactionsYear] = useState<
    Transaction[]
  >([]);
  ...
  const fetchTransactions = async (): Promise<void> => {
    try {
      ...
      const dataYear = await getTransactionsYear({ year });
      setTransactionsYear(dataYear);
      setFilteredTransactionsYear(dataYear);
    } catch (err) {
  ...
  const handleSearchChange = (event: ChangeEvent<HTMLInputElement>): void => {
    ...
    setFilteredTransactionsYear(
      transactionsYear.filter((transaction) =>
        transaction.description
          .toUpperCase()
          .includes(event.target.value.toUpperCase())
      )
  ...
  const handleDelete = async (id: string): Promise<void> => {
    try {
      ...
      setFilteredTransactionsYear((prev) => prev.filter((t) => t.id !== id));
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="divide-y divide-gray-700 in-h-full w-full">
                ...
                <tbody className="divide-y divide-gray-700">
                  {filteredTransactionsYear.map((transaction) => (
                    <tr key={transaction.id} className="hover:bg-gray-800">
                      <td className="px-3 py-4 text-sm text-gray-400 whitespace-nowrap ">
                        <div className="flex items-center">
                          <div className="mr-2">
